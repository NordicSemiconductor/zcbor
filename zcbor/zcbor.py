...
class CodeGenerator(CddlXcoder):
    ...
    def __init__(
        self, mode, entry_type_names, default_bit_size, *args, unordered_maps=False, **kwargs
    ):
    ...
    def init_kwargs(self):
        ...
        return {**(super().init_kwargs()), "unordered_maps": self.unordered_maps}
    ...
    def num_map_search_flags(self):
        ...
        if self.key:
            flags, nested_flags = self.key.num_map_search_flags()
            assert flags == 0, "keys cannot have keys"
            total_nested += nested_flags
            total_this += 1
        if self.cbor_var_condition():
            flags, nested_flags = self.cbor.num_map_search_flags()
            assert flags == 0, "Cannot have bare keys directly within .cbor."
            total_nested += nested_flags
        if self.type in ["LIST", "MAP", "GROUP", "UNION"]:
            c_flags, c_nested_flags = zip(*(child.num_map_search_flags() for child in self.value))
            ...
            if self.type == "MAP":
                ...
                total_nested += (sum(c_flags) + 7) // 8 * 8
            elif self.type == "LIST":
                assert sum(c_flags) == 0, "Cannot have keys in LIST"
            elif self.type == "GROUP":
    ...
    def is_valid_map_elem(self):
        """(Redefinition) Return whether the current element is a valid member of a MAP."""
    ...
    def xcode_union(self):
        ...
        if self.mode == "decode":
            ...
            if len(self.value) > 0 and (
                not self.value[0].key_var_condition() or not self.unordered_maps
            ):
                return (
...
{arg_test}
    ...
    def render_entry_function(self, xcoder, mode):
        ...
        if xcoder.unordered_maps and mode == "decode":
            num_flags = sum(xcoder.num_map_search_flags())
...
the file's contents will be used.""",
    ...
    code_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="""Add support in the generated code for decoding maps with unknown element order.
