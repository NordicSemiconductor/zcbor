...
class CddlParser:
    ...
    def is_valid_map_elem(self):
        ...
        ret = self.key is not None\
            or (self.type == "OTHER" and self.my_types[self.value].is_valid_map_elem())\
            or (self.type in ["GROUP", "UNION"]
                and (self.value and all(child.is_valid_map_elem() for child in self.value)))
        return ret
    ...
    def post_validate(self):
        ...
        if self.type in ["LIST", "MAP"]:
            invalid_elems = [child for child in self.value if not child.is_valid_map_elem()]
            child_keys = [child for child in self.value if child not in invalid_elems]
            if self.type == "MAP" and invalid_elems:
                raise TypeError(
                    "Map member(s) are invalid: " + str(invalid_elems) + " pointing to "
                    + str(
                        [self.my_types[elem.value] for elem in invalid_elems
                            if elem.type == "OTHER"]))
...
class CodeGenerator(CddlXcoder):
    ...
    def num_map_search_flags(self, in_map):
        ...
        total = 0
        if self.key:
            total_ex += sum(self.key.num_map_search_flags(False))
            total += 1
        if self.cbor_var_condition():
            total_ex += sum(self.cbor.num_map_search_flags(False))
        if self.type in ["LIST", "MAP", "GROUP", "UNION"]:
            child_in_map = in_map if self.type in ["GROUP", "UNION"] else self.type == "MAP"
            total_ex += max([sum(child.num_map_search_flags(child_in_map)) for child in self.value])
        if self.type == "OTHER":
            t, t_ex = self.my_types[self.value].num_map_search_flags(in_map)
            total += t
            total_ex += t_ex
        return total * self.max_qty, total_ex
    ...
    def is_valid_map_elem(self):
        if self.unordered_maps:
            if (self.type == "GROUP") and (len(self.value) > 1) \
                    and ((self.max_qty != 1) or (self.min_qty != 1)):
                return False
    ...
    def repeated_xcode(self, union_int=None):
        ...
        if self.key:
            if self.mode == "decode" and self.unordered_maps:
                func, *arguments = self.key.single_func(ptr_result=True)
