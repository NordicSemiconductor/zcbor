		...
		zcbor_state_t *state, const struct Pet *input)
{
	...
	bool res = (((zcbor_list_start_encode(state, 3) && ((((zcbor_list_start_encode(state, 0) && ((zcbor_multi_encode_minmax(1, DEFAULT_MAX_QTY, &(*input).names_count, ZCBOR_CUSTOM_CAST_FP(zcbor_tstr_encode), state, (*&(*input).names), sizeof(struct zcbor_string))) || (zcbor_list_map_end_force_encode(state), false)) && zcbor_list_end_encode(state, 0)))
