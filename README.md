...
options:
  ...
  --unordered-maps      [EXPERIMENTAL] Add support in the generated code for
                        decoding maps with unknown element order. When
                        enabled, the generated code will use the
                        zcbor_unordered_map_*() API to decode data whenever
                        inside a map. zcbor detects when
                        ZCBOR_MAP_SMART_SEARCH is needed and enables it in the
                        generated cmake file. Enabling --unordered-maps places
                        some restrictions on the level of ambiguity allowed
                        between map keys in a map. Only affects decoding
                        (--decode/-d).
