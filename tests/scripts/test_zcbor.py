...
class TestIntSize(PopenTest, TempdTest):
    def do_test_int_size(self, mode, defines_arg, bit_size):
        ...
        expected_defines = [
            "#define INTMAX1_INT_8_MIN_VAL (INT8_MIN)",
            "#define INTMAX1_INT_8_MAX_VAL (INT8_MAX)",
            "#define INTMAX1_UINT_8_MAX_VAL (UINT8_MAX)",
            "#define INTMAX1_INT_16_MIN_VAL (INT16_MIN)",
            "#define INTMAX1_INT_16_MAX_VAL (INT16_MAX)",
            "#define INTMAX1_UINT_16_MAX_VAL (UINT16_MAX)",
            "#define INTMAX1_INT_32_MIN_VAL (INT32_MIN)",
            "#define INTMAX1_INT_32_MAX_VAL (INT32_MAX)",
            "#define INTMAX1_UINT_32_MAX_VAL (UINT32_MAX)",
            "#define INTMAX1_INT_64_MIN_VAL (INT64_MIN)",
            "#define INTMAX1_INT_64_MAX_VAL (INT64_MAX)",
            "#define INTMAX1_UINT_64_MAX_VAL (UINT64_MAX)",
            "#define INTMAX4_INT_8_MIN_VAL (INT8_MIN)",
            "#define INTMAX4_INT_8_MAX_VAL (INT8_MAX)",
            "#define INTMAX4_UINT_8_MAX_VAL (UINT8_MAX)",
            "#define INTMAX4_INT_16_MIN_VAL (INT16_MIN)",
            "#define INTMAX4_INT_16_MAX_VAL (INT16_MAX)",
            "#define INTMAX4_UINT_16_MAX_VAL (UINT16_MAX)",
            "#define INTMAX4_INT_32_MIN_VAL (INT32_MIN)",
            "#define INTMAX4_INT_32_MAX_VAL (INT32_MAX)",
            "#define INTMAX4_UINT_32_MAX_VAL (UINT32_MAX)",
            "#define INTMAX4_INT_64_MIN_VAL (INT64_MIN)",
            "#define INTMAX4_INT_64_MAX_VAL (INT64_MAX)",
            "#define INTMAX4_UINT_64_MAX_VAL (UINT64_MAX)",
            "#define INTMAX5_INT_8_MIN_PLUS1_VAL (-129)",
            "#define INTMAX5_INT_8_MAX_PLUS1_VAL (128)",
            "#define INTMAX5_UINT_8_MAX_PLUS1_VAL (256)",
            "#define INTMAX5_INT_16_MIN_PLUS1_VAL (-32769)",
            "#define INTMAX5_INT_16_MAX_PLUS1_VAL (32768)",
            "#define INTMAX5_UINT_16_MAX_PLUS1_VAL (65536)",
            "#define INTMAX5_INT_32_MIN_PLUS1_VAL (-2147483649)",
            "#define INTMAX5_INT_32_MAX_PLUS1_VAL (2147483648)",
            "#define INTMAX5_UINT_32_MAX_PLUS1_VAL (4294967296ULL)",
        ]
        ...
        if defines_arg == []:
            ...
            extract_define = compile(r"#define (?P<name>\w+) \((?P<value>[^\(\)]+)\)")
            extracted_defines = [extract_define.match(e) for e in expected_defines]
            defines_regexes = [
                (compile(em.group("name")), em.group("value")) for em in extracted_defines
            ]
            expected_output_code_lit = []
            for e in expected_output_code:
                e2 = e
                for er, ev in defines_regexes:
                    e2 = er.sub(ev, e2)
        ...
        if defines_arg == []:
            for e in expected_defines:
                self.assertNotIn(e, output_h, f"Expected no '{e}' in output_h")
            for e in expected_output_code_lit:
                self.assertIn(e, output_c, f"Expected '{e}' in output_c")
        else:
            for e in expected_defines:
                self.assertIn(e, output_h, f"Expected '{e}' in output_h")
            for e in expected_output_code:
                self.assertIn(e, output_c, f"Expected '{e}' in output_c")
