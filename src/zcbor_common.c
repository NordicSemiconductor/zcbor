...
bool zcbor_new_backup_w_elem_state(zcbor_state_t *state, size_t new_elem_count, bool backup_elem_state)
{
	...
	PRINT_FUNC_ARGS("(backup_elem_state=%u, backup_num=%zu)", backup_elem_state, state->constant_state->current_backup+1);
	zcbor_log("\r\n");
	ZCBOR_CHECK_ERROR();
	...
	size_t n_states, size_t elem_count, size_t n_elem_states)
{
	...
	if (n_elem_states > 0) {
		...
		flags =	(uint8_t *)&states[flag_state_index];
	...
	zcbor_new_state(states, n_states, payload, payload_len, elem_count, flags,
			n_elem_state_bytes);
	...
	size_t n_states, size_t elem_count)
{
	return zcbor_entry_function_with_elem_states(payload, payload_len, result, payload_len_out,
		states, func, n_states, elem_count, 0);
}
