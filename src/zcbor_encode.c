			...
			zcbor_major_type_t major_type)
{
	...
	size_t hint_header_len = zcbor_header_len_ptr(&size_hint, sizeof(size_hint));
	size_t header_len = zcbor_header_len_ptr(&list_count, sizeof(list_count));

	/** This resets the payload pointer to before the list/map header.
	  * For that reason the previous payload was stored above. */
	if (!zcbor_process_backup(state, ZCBOR_FLAG_RESTORE | ZCBOR_FLAG_CONSUME, ZCBOR_MAX_ELEM_COUNT)) {
		ZCBOR_FAIL();
	}

	const uint8_t *old_start = state->payload + hint_header_len;
	uint8_t *new_start = state->payload_mut + header_len;
	size_t body_size = (size_t)payload - (size_t)old_start;

	if (payload < old_start) {
		ZCBOR_ERR(ZCBOR_ERR_BAD_ARG);
	}

	/** memmove before encoding the header since encoding the header
	  * will corrupt the data if the encoded header is larger than the
	  * previously encoded header.
	...
	if (new_start != old_start) {
		if ((new_start + body_size) > state->payload_end) {
			ZCBOR_ERR(ZCBOR_ERR_NO_PAYLOAD);
		}
		memmove(new_start, old_start, body_size);
	}
